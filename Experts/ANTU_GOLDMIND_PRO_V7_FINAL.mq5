//+------------------------------------------------------------------+
//|                              ANTU_GOLDMIND_PRO_V7_FINAL.mq5      |
...
input double   InpMaxSLDollar        = 3.0;      // MAX SL $ per trade (hard cap)

//================ SL / TP =========================//
input group "=== Stop Loss / Take Profit ==="
input bool     InpUseATRStops        = true;     // Use ATR-based SL/TP
input double   InpATRMultSL          = 1.2;      // ATR x for SL (tight)
input double   InpATRMultTP          = 3.5;      // ATR x for TP (wide)
...
input double   InpBEActivatePips     = 7.0;      // Activate BE after +X pips
input double   InpBEOffsetPips       = 2.0;      // Lock +X pips at BE
input bool     InpUseTrailing        = true;     // Trail stop
input double   InpTrailStartPips     = 10.0;     // Start trail after +X pips
input double   InpTrailStepPips      = 5.0;      // Trail distance pips
...
input double   InpPartialTPRatio     = 0.5;      // Close at X of full TP
...
input double   InpRSIBuyLevel        = 35.0;     // RSI Buy level (below = buy)
input double   InpRSISellLevel       = 65.0;     // RSI Sell level (above = sell)
...
input string   InpComment            = "Goldmind V7 Final";
...
bool ValidateLicense(){
   ...
   if(InpTrialDays > 0){
      ...
      else { SendNotify("ANTU EA: Trial EXPIRED!"); return false; }
   }
   return false;
}

//+------------------------------------------------------------------+
//| Count OUR positions (magic + symbol filtered)                    |
//+------------------------------------------------------------------+
...
void UpdateDashboard(double pnl,int spread,double band_dist,double atrPips,string status){
   ...
   DrawText("UI_Title",45,40,"ANTU GOLDMIND PRO V7",11,C'15,15,15',"Segoe UI Black",true);
...
//+------------------------------------------------------------------+
int OnInit(){
   if(!ValidatePassword()){ Alert("ANTU EA: Invalid Password!"); return(INIT_FAILED); }
   if(admin_mode){ ShowKeyGeneratorPanel(); return(INIT_SUCCEEDED); }
   license_valid = false;
   if(!ValidateLicense()){ Alert("ANTU EA: License expired!"); return(INIT_FAILED); }
   ...
   ParseNewsTimes();
   long accNum = AccountInfoInteger(ACCOUNT_LOGIN);
   SendNotify("ANTU EA V7 started! Acc:"+IntegerToString(accNum)+" MaxSL:$"+DoubleToString(InpMaxSLDollar,2));
...
void ParseNewsTimes(){
   ...
   for(int i=0;i<n;i++){
      ...
      MqlDateTime nt = now;
      nt.hour=(int)StringToInteger(hm[0]);
      nt.min=(int)StringToInteger(hm[1]);
      nt.sec=0;
      int sz = ArraySize(news_times_today);
      ArrayResize(news_times_today,sz+1);
      news_times_today[sz]=StructToTime(nt);
   }
}
...
void RefreshDailyStats(){
   ...
   double profit = 0.0; int trades = 0; int consec = 0;
   for(int i=0;i<deals;i++){
      ...
      profit += p;
      if(entry == DEAL_ENTRY_OUT){ trades++; if(p<0) consec++; else consec=0; }
   }
   cached_daily_profit=profit; cached_daily_trades=trades;
   cached_consec_losses=consec; last_deals_total=deals;
}
...
void ManageOpenPositions(){
   for(int i=PositionsTotal()-1;i>=0;i--){
      ...
      if(type==POSITION_TYPE_BUY){
         ...
         // Partial Close
         if(InpUsePartialClose && tpPips>0 && profitPips >= tpPips*InpPartialTPRatio){
            ...
            if(closeLot>=minLot && vol>minLot){
               ...
               if(StringFind(comment,"PC")<0){
                  ...
                  if(InpNotifyOnTrade) SendNotify("ANTU: PARTIAL CLOSE BUY +"+DoubleToString(profitPips,1)+"pip");
                  continue;
               }
            }
         }
         ...
         if(InpUsePartialClose && tpPips>0 && profitPips >= tpPips*InpPartialTPRatio){
            ...
            if(closeLot>=minLot && vol>minLot){
               ...
               if(StringFind(comment,"PC")<0){
                  trade.PositionClosePartial(ticket, closeLot);
                  if(InpNotifyOnTrade) SendNotify("ANTU: PARTIAL CLOSE SELL +"+DoubleToString(profitPips,1)+"pip");
                  continue;
               }
            }
         }
...
void OnTick(){
   ...
   if(targetUSD>0 && cached_daily_profit>=targetUSD){
      status="TARGET HIT"; blocked=true; NotifyBlock("Target hit");
   }
   else if(maxLossUSD>0 && cached_daily_profit<=-maxLossUSD){
      status="LOSS HIT"; blocked=true; NotifyBlock("Loss limit hit");
   }
